...
import useAllGemOrigins from "../../../hooks/useAllGemOrigins";
...
function DataTable() {
  ...
  const allGemOrigins = useAllGemOrigins(filterData);
  ...
  const deleteNewsContent = (id) => {
    confirm({
      ...
      onOk() {
        Progress.show();
        axiosInstance
          .delete(`${apis.GEMSTONE_ORIGIN}/${id}`)
          .then((result) => {
            let responseJson = result;
            setRefreshTable(!refreshTable);
            Progress.hide();
  ...
  return (
    <>
      <Table
        scroll={{ x: 1000 }}
        columns={columns}
        dataSource={allGemOrigins.data}
        bordered
        pagination={{
          current: filterData.current_page,
          pageSize: filterData.pageSize,
          total: allGemOrigins.total,
