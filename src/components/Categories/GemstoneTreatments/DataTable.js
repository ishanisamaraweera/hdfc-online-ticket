...
import useAllGemTreatments from "../../../hooks/useAllGemTreatments";
...
function DataTable() {
  ...
  const allGemTreatments = useAllGemTreatments(filterData);
  ...
  const statusChange = (id, type) => {
    confirm({
      ...
      onOk() {
        Progress.show();
        axiosInstance
          .put(`${apis.GEMSTONE_TREATMENT}/${id}`, {
            status: type,
          })
          .then((result) => {
  ...
  const deleteNewsContent = (id) => {
    confirm({
      ...
      onOk() {
        Progress.show();
        axiosInstance
          .delete(`${apis.GEMSTONE_TREATMENT}/${id}`)
          .then((result) => {
            let responseJson = result;
            setRefreshTable(!refreshTable);
            Progress.hide();
  ...
  return (
    <>
      <Table
        scroll={{ x: 1000 }}
        columns={columns}
        dataSource={allGemTreatments.data}
        bordered
        pagination={{
          current: filterData.current_page,
          pageSize: filterData.pageSize,
          total: allGemTreatments.total,
